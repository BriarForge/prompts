...
4. `[SKILL_PATH]` exists. Read it end to end before touching anything else.
5. `next_candidates` in `[STATE_JSON]` is non-empty. If it is empty, the sweep queue is exhausted; STOP and ping Mcored.
6. No other agent has posted a "starting, HEAD = <sha>" to #xerahs in the last 30 minutes. If someone did, ping them in-thread to confirm whether they finished or stalled before you start.
...
- `[AGENT_BRANCH]` — operator's working branch (typically `[AGENT_REMOTE]/develop`). Verify with `git branch -r | grep [AGENT_REMOTE]`.
